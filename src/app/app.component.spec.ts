...
describe('AppComponent', () => {
  ...
  it('should recursively set the url property to omnibar.nav.services.items', async(() => {
    ...
    skyAppConfig.runtime.params.getUrl = (url: string) => url + '?svcid=abc';

    setup(skyAppConfig, false).then(() => {
      fixture.detectChanges();
      const items = spyOmnibar.calls.first().args[0].nav.services[0].items;
      expect(items[0].url).toEqual('ignored.com');
      expect(items[1].url).toEqual('base.com/custom-base/custom-route?svcid=abc');
      expect(items[2].items[0].url).toEqual('base.com/custom-base/another-custom-route?svcid=abc');
  ...
  it('should add global public routes as localNavItems during serve', async(() => {
    ...
    skyAppConfig.runtime.params.getUrl = (url: string) => url + '?svcid=123';

    setup(skyAppConfig, false).then(() => {
      fixture.detectChanges();
      expect(spyOmnibar.calls.first().args[0].nav.localNavItems[0]).toEqual({
        title: 'my-name',
        url: 'base.com/custom-base/my-route?svcid=123',
        data: {
          global: true,
          name: 'my-name',
          route: '/my-route'
