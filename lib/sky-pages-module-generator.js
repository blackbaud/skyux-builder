...
`;
  ...
  const formattedDeclarations = names.join(`,\n${codegen.indent(2)}`);
  const formattedImports = runtimeModuleImports.join(`,\n${codegen.indent(2)}`);
  const formattedExports = runtimeModuleExports.join(`,\n${codegen.indent(2)}`);
  const formattedProviders = runtimeProviders.join(`,\n${codegen.indent(2)}`);
