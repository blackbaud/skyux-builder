...
function getSource(skyAppConfig) {
  ...
  // Generate style `require()` statements.
  const styles = skyAppConfig.skyux.app.styles && skyAppConfig.skyux.app.styles
    .filter(path => !/^http/.test(path))
    .map(path => `require('style-loader!${path}');\n`);
