...
function getSource(skyPagesConfig) {
  ...
  console.log('previous sky-pages-config', skyPagesConfig.includeRouteModule);
  ...
  let names = '';
  if (skyPagesConfig.includeRouteModule) {
    names = `${componentNames.concat(routes.names).join(',\n    ')}`;
  } else {
    names = componentNames.join(', \n    ');
  }
  ...
  console.log('includeRouteModule', skyPagesConfig.includeRouteModule);

  if (skyPagesConfig.includeRouteModule) {

    moduleImports.push('routing');

    moduleProviders.push('appRoutingProviders');
    moduleProviders.push(`{
    ...
    /*
        Import the regular RouterModule so that tested components can reference things like routerLink
    */
    moduleImports.push('RouterModule');
  }
...
import { SkyAppWindowRef } from '${skyPagesConfig.runtimeAlias}';
${authHttpImport}
