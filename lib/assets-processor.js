...
function processAssets(content, baseUrl, callback) {
  ...
  while (match) {
    ...
    let filePath = matchString.substring(2, matchString.length);

    const isLocaleFile = /(\/|\\)locales(\/|\\)resources_/.test(filePath);

    // Locale files are compiled early in the build and
    // added to a temporary directory. We need to tell the asset loader
    // where the physical files can be found.
    if (isLocaleFile) {
      filePath = spaPath('.skypageslocales', path.basename(filePath));
    }

    let filePathWithHash = assetsUtils.getFilePathWithHash(filePath, true);

    // Adjust the hashed file's path to point where it *should* be located.
    if (isLocaleFile) {
      filePathWithHash = path.join('assets', 'locales', path.basename(filePathWithHash));
    }

    if (callback) {
      callback(filePathWithHash, spaPath('src', filePath));
    }
